    ...
    public partial class GenHotfixDelegate
    {
        ...
        static void GetClassName(System.Type type, out string clsName, out string realClsName, out bool isByRef, bool simpleClassName = false)
        {
            ...
            if (type.IsNested)
            {
                ...
                if (rt == type)
                {
                    L.LogErrorFormat("type:{0} -> {1}", type.Name, rt.Name);
                }
            ...
            if (type.IsGenericType && !isNestedGeneric)
            {
                ...
                foreach (var j in args)
                {
                    ...
                    if (type == j)
                    {
                        L.LogErrorFormat("type:{0} -> {1}", type.Name, j.Name);
                    }
...
";
        ...
        //    CsHotFix.L.LogFormat(methods.Length.ToString());
        //}
        ...
        [MenuItem("CsHotFix/手动操作/1. 注册需要热更的类",false,1)]
        static void Gen()
        {
            AutoCode(GenAutoExport.FixMarkIL());
        }

        [MenuItem("CsHotFix/手动操作/清除需要热更的类",false,3)]
        static void Clear()
        {
        ...
        public static void AutoCode(List<string> classes)
        {
            ...
            foreach (var ator in System.AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    if (exports.Contains(type.FullName.Replace('+', '/')) || type.GetCustomAttributes(typeof(CsHotAttribute), false).Length != 0)
                    {
                        //CsHotFix.L.LogFormat("type:{0}", type.FullName);
                        if (type.IsGenericType)
                            continue;
            ...
            string file = string.Format( EditorConstant.CsHotfixOut + "GenDelegateBridge_{0}.cs", suffix);
