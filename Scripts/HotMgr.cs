...
namespace CsHotFix
{
    ...
    public class Hotfix
    {
        ...
        public Hotfix(System.Type type, string name, System.Type hotfixType) : this(type, "__Hotfix_" + name, name,
            hotfixType, name)
        {
        }
        ...
        {
            field = type.GetField(fieldName, HotMgr.bindingFlags);
            if (field == null)
            {
                L.LogErrorFormat("type field:{0}.{1} not find!", type.Name, fieldName);
                return;
            }
            ...
            if (method == null)
            {
                L.LogErrorFormat("type method {0}.{1} not find!", type.Name, funName);
                return;
            }

            var hotMethod = hotfixType.GetMethod(hotfixFunName, HotMgr.bindingFlags);
            if (hotMethod == null)
            {
                L.LogErrorFormat("{0}.{1} Replace {2}.{3}", type.Name, funName, hotfixType.Name, hotfixFunName);
                return;
            }

            if (!HotMgr.IsStatic(hotMethod))
            {
                L.LogErrorFormat("Hotfix type:{0} funName:{1} methodInfo is not static!", hotfixType.Name,
                    hotfixFunName);
                return;
            }
            ...
            L.LogFormat("{0}.{1} Replace {2}.{3}", type.Name, funName, hotfixType.Name, hotfixFunName);
        }
    ...
    {
        public static System.Text.StringBuilder AppendFormatLine(this System.Text.StringBuilder sb, string format,
            params object[] objs)
        {
        ...
        public static void Init(IResLoad resLoad = null)
        {
            ...
            {
#if UNITY_EDITOR
                resLoad = new EditorResLoad();
#endif
            }

            if (!InitHotModule(resLoad)) return;
            ...
            InitByProperty(typeof(AutoInitAndRelease), "Init", AllTypes); // run init method
        }
        ...
        static public bool InitHotModule(IResLoad resLoad)
        {
            ...
            try
            {
                DllStream = CopyStream(resLoad.GetDllStream());
                {
#if USE_PDB
                    SymbolStream = CopyStream(resLoad.GetPDBStream());
                    appdomain.LoadAssembly(DllStream, SymbolStream, new ILRuntime.Mono.Cecil.Pdb.PdbReaderProvider());
#elif USE_MDB
                    SymbolStream = CopyStream(resLoad.GetMDBStream());
            ...
            {
                Debug.LogError("init il error." + ex.Message);
                return false;
            }
            ...
            // List<T>
            {
                ...
                appdomain.DelegateManager
                    .RegisterDelegateConvertor<System.Comparison<ILRuntime.Runtime.Intepreter.ILTypeInstance>>((act) =>
                    {
                        ...
                        {
                            return ((System.Func<ILRuntime.Runtime.Intepreter.ILTypeInstance,
                                ILRuntime.Runtime.Intepreter.ILTypeInstance, int>) act)(x, y);
                        });
                    });
            }
        ...
        static bool GetReplaceFunction(Collection<CustomAttribute> CustomAttributes, out System.Type type,
            out string fieldName)
        {
            ...
            for (int i = 0; i < CustomAttributes.Count; ++i)
            {
                ...
                if (custom.AttributeType.FullName != "CsHotFix.ReplaceFunction")
                    continue;
        ...
        static void GetPlatform(Collection<CustomAttribute> CustomAttributes,
            System.Action<UnityEngine.RuntimePlatform> action)
        {
            CustomAttribute custom;
            for (int i = 0; i < CustomAttributes.Count; ++i)
            {
                custom = CustomAttributes[i];
                if (custom.AttributeType.FullName != "CsHotFix.Platform")
                    continue;
        ...
        static System.Type GetReplaceType(Collection<CustomAttribute> CustomAttributes)
        {
            ...
            {
                custom = CustomAttributes[i];
                if (custom.AttributeType.FullName != "CsHotFix.ReplaceType")
                    continue;
        ...
        public static bool ReplaceField(System.Type type, string fieldName, MethodInfo info)
        {
            ...
            if (field == null)
            {
                L.LogErrorFormat("ReplaceField type:{0} fieldName:{1} not find!", type.Name, fieldName);
                return false;
            }
            ...
            {
                field.SetValue(null, null);
                L.LogFormat("ReplaceFunction type:{0} fieldName:{1} Cannel!", type.Name, fieldName);
            }
            else
            {
                if (!IsStatic(info))
                {
                    L.LogErrorFormat("ReplaceField type:{0} fieldName:{1} methodInfo is not static!", type.Name,
                        fieldName, info.Name);
                    return false;
                }

                field.SetValue(null, new global::IL.DelegateBridge(info));
                L.LogFormat("ReplaceFunction type:{0} fieldName:{1}", type.Name, fieldName);
            }
        ...
        static void AutoReplace(List<IType> types)
        {
            ...
            foreach (var ator in types)
            {
                ...
                {
                    L.LogWarningFormat("platorms:{0} type:{1} not hotfix!", rp, type.Name);
                    continue; // 不属于此平台的
                }
                ...
                foreach (var il in type.GetMethods())
                {
                    ...
                    {
                        L.LogErrorFormat("type:{0} method:{1} is not static fun!", type.Name, method.Name);
                        continue;
                    }
                    ...
                    if (platforms.Count != 0 && !platforms.Contains(rp))
                    {
                        L.LogWarningFormat("platorms:{0} type:{1}.{2} not hotfix!", rp, type.Name, ilMethod.Name);
                        continue; // 不属于此平台的
                    }
                    ...
                    if (srcType == null)
                    {
                        L.LogErrorFormat("type:{0} method:{1} not set srcType!", type.Name, method.Name);
                        continue;
                    }
                    ...
                    if (field == null)
                    {
                        // type.Name 未找到该类型.(检查是否打 HotFix 标签)
                        Debug.LogErrorFormat("hotType:{0} method:{1} not find srcType:{2}.{3} hot field!", type.Name,
                            method.Name, srcType.FullName, fieldName);
                        continue;
                    }
                    ...
                    field.SetValue(null, bridge);
                    Fields.Add(field);
                    L.LogFormat("type:{0} method:{1} Replace {2}.{3}!", type.Name, method.Name, srcType.Name,
                        fieldName);
            ...
            foreach (var itor in types)
            {
                ...
                foreach (var att in td.CustomAttributes)
                {
                    if (att.AttributeType.FullName == autoAttName)
                    {
                        ...
                        if (!method.IsStatic)
                        {
                            L.LogErrorFormat("hot type:{0} method:{1} not static!", ilType.FullName, name);
                            continue;
                        }
        ...
        static void AutoSetFieldMethodValue(System.Type srcType, FieldInfo srcFieldInfo, ILType type, string fieldName,
            DelegateBridge bridge, Dictionary<string, List<MethodInfo>> NameToSorted)
        {
            var auto_fieldInfo = FindStaticField(type, fieldName);
            if (auto_fieldInfo == null || auto_fieldInfo.FieldType.FullName != "CsHotFix.Hotfix")
                return;

            var fieldInfo = type.ReflectionType.GetField(fieldName, bindingFlags);
            if (fieldInfo == null)
            {
                L.LogErrorFormat("type:{0} fieldInfo:{1} find but not find clrType fieldInfo!", type.Name, fieldName);
                return;
            }
            ...
            MethodInfo srcMethodInfo = null;
            {
                ...
                {
                    L.LogErrorFormat("type:{0} method:{1} not find by Field!", type.Name, methodName, fieldName);
                    return;
                }
            }
