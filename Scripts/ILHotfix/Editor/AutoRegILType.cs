    ...
    static partial class AutoRegILType
    {
        ...
        static bool IsEditorTypeImp(System.Type type)
        {
            ...
            if (!string.IsNullOrEmpty(ns) && (ns.StartsWith("GUIEditor.") || ns.StartsWith("UnityEditor.") ||
                                              ns.StartsWith("MikuArtTech.EditorTools.")))
                return true;
        ...
        static bool IsHotUsed(System.Type type)
        {
            ...
            if ( /*fullName.StartsWith("Config.") ||*/
                ...
                fullName.Contains("CsHotFix.IModleFactory") ||
                ...
                //fullName.Contains("CsHotFix.Draw") ||
                //fullName.Contains("CsHotFix.RenderCache") ||
                //fullName.Contains("CsHotFix.Line") ||
                ...
                //fullName.Contains("CsHotFix.ClothHandle") ||
                //fullName.Contains("CsHotFix.ClothConfig") ||
                //fullName.Contains("CsHotFix.HairItem") ||
                //fullName.Contains("CsHotFix.ClothItem") ||
                //fullName.Contains("CsHotFix.WeaponItem") ||
                //fullName.Contains("CsHotFix.RideItem") ||
        ...
        [UnityEditor.MenuItem("CsHotFix/手动操作/2. 委托自动生成", false, 2)]
        public static void Build()
        {
            HashSet<System.Type> types = new HashSet<System.Type>();
            var assemblies = System.AppDomain.CurrentDomain.GetAssemblies();
#if USE_HOT && UNITY_EDITOR
            var tps = ReflectionUtil.FindSubClassesInAllAssemblies<IResLoad>();
            foreach (var resLoad in tps)
            {
                try
                {
                    Assembly assembly = Assembly.LoadFile(resLoad.DllPath());
                    types.UnionWith(assembly.GetTypes());
                }
                catch (FileNotFoundException e)
                {
                    Debug.Log(resLoad.DllPath() + " load error,file is not exist!");
                    continue;
                }
                Debug.Log("委托生成:" + resLoad.DllPath());
            }
#endif
        ...
        [UnityEditor.MenuItem("CsHotFix/手动操作/清除委托生成脚本", false, 4)]
        ...
        static void Export(HashSet<System.Type> types)
        {
            ...
            Dictionary<System.Type, UseTypeInfo>
                csharpDelegate = new Dictionary<System.Type, UseTypeInfo>(); // C#层当中的的所有委托类型
            ...
            // 先把C#层的委托注册到管理器当中
            L.LogFormat("csharpDelegate Count:{0} hotTDic:{1}", csharpDelegate.Count, hotTDic.Count);
            BuildDelegate(csharpDelegate, hotTDic);
        }

        //[UnityEditor.MenuItem("CsHotFix/TestBuildCode")]
        static void TestBuildCode()
        {
            var tests = new HashSet<System.Type>(new System.Type[]
            {
                ...
                //IL.Help.GetTypeByFullName("CsHotFix.ClothConfig"),                
            ...
            public void To(System.Text.StringBuilder sb, string suffix)
            {
                foreach (var info in infos)
                {
                    ...
                    if (info is MethodInfo)
                    {
                        ...
                        sb.Append(string.Format("{0} {1}.{2}(", GetClassRealClsName(m.ReturnType),
                            GetClassRealClsName(m.ReflectedType), m.Name));
                        if (ps.Length == 0)
                            sb.Append(");");
                        else
                        {
                    ...
                    else if (info is FieldInfo)
                    {
                        ...
                        sb.AppendFormat("{0} {1}.{2}", GetClassRealClsName(f.FieldType),
                            GetClassRealClsName(info.DeclaringType), f.Name);
                        sb.AppendLine();
                    }
                    else if (info is PropertyInfo)
                    {
                        var p = (PropertyInfo) info;
                        sb.AppendFormat("{0} {1}.{2}", GetClassRealClsName(p.PropertyType),
                            GetClassRealClsName(p.PropertyType), p.Name);
                        sb.AppendLine();
                    }
                    ...
                    {
                        L.LogErrorFormat("info:{0}", info.GetType().FullName);
                    }
                }
            }
        }
        ...
        static void BuildType(System.Type type, HashSet<System.Type> hotTypes,
            Dictionary<System.Type, UseTypeInfo> csharpDelegate, HashSet<System.Type> Checks)
        {
        ...
        static void CheckMethods(System.Type type, HashSet<System.Type> hotTypes,
            Dictionary<System.Type, UseTypeInfo> csharpDelegate, HashSet<System.Type> Checks)
        {
        ...
        static void BuildDelegate(Dictionary<System.Type, UseTypeInfo> csharpDelegate,
            Dictionary<System.Type, UseTypeInfo> hotTDic)
        {
            string marco, file_suffix;
            GetPlatform(out marco, out file_suffix);
            string file_path = string.Format(EditorConstant.CsHotfixOut + "ILRegType_{0}.cs", file_suffix);
            ...
            foreach (var ator in csharpDelegate)
            {
                ...
                if (method == null)
                {
                    L.LogErrorFormat("method:{0}", d.FullName);
                    continue;
                }
        ...
        static void BuildDHot(Dictionary<System.Type, UseTypeInfo> hotTDic, System.Text.StringBuilder sb)
        {
            ...
            foreach (var d in hotTDic)
            {
                ...
                if (d.Key.FullName.StartsWith("System.Collections.Generic.Dictionary`2[["))
                {
                    ...
                    sb.AppendLine(string.Format("v = typeof(System.Collections.Generic.Dictionary<{0},{1}>);",
                        GetTypeByHot(parameter[0]), GetTypeByHot(parameter[1])));
                }
                else
                {
