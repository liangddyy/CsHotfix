    ...
    public class GenAutoExport
    {
        ...
        public static List<string> FixMarkIL()
        {
            ...
            foreach (var t in types)
            {
                ...
                // 跳过非HotFix
                if (!IsAttributes(t, typeof(CsHotAttribute)))
                {
                    continue;
                }

                if (t.GetCustomAttributes(typeof(CsHotAttribute), false).Length == 0)
                {
                    //ILRuntime
                    string FullName = t.FullName;
                    if (FullName.StartsWith("IL.") ||
                        FullName.StartsWith("ILRuntime.") ||
                        FullName.StartsWith("CsHotFix.") ||
            ...
            L.Log(string.Format("Find {0} Classes {1} Delegates.", classes.Count, delegates.Count));
            return classes;
        }
    }
}
